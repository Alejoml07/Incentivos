...
namespace Usuarioos
{
  ...
  [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "fidelizacion/SincronizarLiquidacionPuntos")] HttpRequest req, ILogger log)
        ...
        [FunctionName("GetGarantiasByUser")]
        [OpenApiOperation(operationId: "GetGarantiasByUser", tags: new[] { "GetGarantiasByUser" })]
        [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "text/plain", bodyType: typeof(GenericResponse<>), Description = "Obtiene los reportes por fechas")]
        public async Task<IActionResult> GetGarantiasByUser(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "fidelizacion/GetGarantiasByUser/{email}")] HttpRequest req, ILogger log,string email)
        {
            log.LogInformation("C# HTTP trigger function processed a request.");

            try
            {
                var response = await this.puntosApplication.GetGarantiasByUser(email);
                return new OkObjectResult(response);
            }
            catch (Exception ex)
            {
